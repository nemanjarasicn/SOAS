...
/*
  it('should query endpoint with initial', (done) => {
    ...
    const source = new PaginatedDataSource<User, UserQuery>(spy, sort, query);
    expect(spy).not.toHaveBeenCalled();
    source.connect().subscribe((users) => {
      expect(users).toEqual(page.content);
      expect(spy).toHaveBeenCalledWith({ page: 0, size: 14, sort, offset: 0 }, query);
      done();
    });
  });
  ...
  it('should query endpoint with inputs', (done) => {
    ...
    const source = new PaginatedDataSource<User, UserQuery>(
      spy,
      initialSort,
      initialQuery,
      1
  ...
  it('should update pagesize', () => {
    ...
    expect(spy).toHaveBeenCalledWith({ page: 0, size: 14, sort }, query);
    source.fetch(1); // (1, 30);
    expect(spy).toHaveBeenCalledWith({ page: 1, size: 14, sort }, query);
    source.fetch(2);
    expect(spy).toHaveBeenCalledWith({ page: 2, size: 14, sort }, query);
